/****************************************************************************
 * arch/loongarch/src/common/loongarch_addrenv.c
 *
...
#define ENTRIES_PER_PGT     (LA_MMU_PAGE_ENTRIES)
...
static_assert((ADDRENV_VBASE & LA_MMU_SECTION_ALIGN_MASK) == 0,
...
static void map_spgtables(group_addrenv_t *addrenv, uintptr_t vaddr)
{
  ...
  prev = loongarch_pgvaddr(addrenv->spgtables[0]);
  ...
  for (i = 0; i < (ARCH_SPGTS - 1); i++)
    {
      uintptr_t next = loongarch_pgvaddr(addrenv->spgtables[i + 1]);
      mmu_ln_setentry(i + 1, prev, next, vaddr, MMU_DIRECTORY_FLAGS);
      prev = next;
    }
}
...
{
  uintptr_t user_mappings = loongarch_pgvaddr(addrenv->spgtables[0]);
  ...
  memcpy((void *)user_mappings, (void *)g_kernel_mappings, LA_MMU_16K_PAGE_SIZE);
                         ...
                         size_t size, uint32_t mmuflags)
{
  ...
  ptprev    = loongarch_pgvaddr(addrenv->spgtables[ARCH_SPGTS - 1]);
  ptlevel   = ARCH_SPGTS;
  ...
  for (i = 0; i < npages; i += ENTRIES_PER_PGT)
    {
      ...
      if (!paddr)
        {
          ...
          mmu_ln_setentry(ptlevel, ptprev, paddr, vaddr, MMU_DIRECTORY_FLAGS);
      ...
      ptlast = loongarch_pgvaddr(paddr);
...
{
/**
 * textsize = 0x1a3e8 < 128*16K = 0x200000 , datasize = 0x588 < 128*16K = 0x200000 
 * comes from file, while heapsize is defined as CONFIG_ARCH_HEAP_NPAGES * pagesize = 0x200000 
 */
  int       ret;
  uintptr_t resvbase;
  uintptr_t resvsize;
  ...
  /* Create the static page tables, 2 static pages is enough for 512G */
  ...
  // /* Loongarch do not need to map the kernel memory for the user, 'cause it got pgdh & pgdl */
...
int up_addrenv_destroy(group_addrenv_t *addrenv)
{
  ...
  ptprev = (uintptr_t *)loongarch_pgvaddr(addrenv->spgtables[ARCH_SPGTS - 1]);
  if (ptprev)
    {
      for (i = 0; i < ENTRIES_PER_PGT; i++)
        {
          ptlast = (uintptr_t *)loongarch_pgvaddr(mmu_pte_to_paddr(ptprev[i]));
          if (ptlast)
            {
  ...
  if (oldenv)
    {
      ...
      uintptr_t pgdh_reg = READ_CSR64(LOONGARCH_CSR_PGDH);
      *oldenv = (save_addrenv_t)pgdh_reg;
    }
...
{
  DEBUGASSERT(oldenv);
  qemu_la_set_pgdh((uintptr_t)*oldenv);
  return OK;
}
