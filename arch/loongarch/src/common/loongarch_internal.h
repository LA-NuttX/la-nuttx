...
#ifdef CONFIG_ARCH_LA32
#  define REGLOAD   __STR(ld.w)
#  define REGSTORE  __STR(st.w)
#else
...
/* LoongArch IOCSR read/write*/
static inline unsigned int iocsr_read32(unsigned int reg)
{
	return __iocsrrd_w(reg);
}

static inline unsigned long iocsr_read64(unsigned int reg)
{
...
uintptr_t *loongarch_doirq(int irq, uintptr_t *regs);
...
/* SYS call 1:
 *
 * void loongarch_fullcontextrestore(uintptr_t *restoreregs) noreturn_function;
 */

#define loongarch_fullcontextrestore(restoreregs) \
  sys_call1(SYS_restore_context, (uintptr_t)restoreregs)

/* SYS call 2:
 *
 * void loongarch_switchcontext(uintptr_t *saveregs, uintptr_t *restoreregs);
 */

#define loongarch_switchcontext(saveregs, restoreregs) \
