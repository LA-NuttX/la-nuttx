...
{
    LA_MMU_L1_PAGE_SIZE, LA_MMU_L2_PAGE_SIZE, LA_MMU_L3_PAGE_SIZE
};
#endif
                     ...
                     uintptr_t vaddr, uint64_t mmuflags)
{
  ...
  DEBUGASSERT(ptlevel > 0 && ptlevel <= LA_MMU_PT_LEVELS);

  /* Calculate index for lntable */

  index = (vaddr >> LA_MMU_VADDR_SHIFT(ptlevel)) & LA_MMU_VPN_MASK;
