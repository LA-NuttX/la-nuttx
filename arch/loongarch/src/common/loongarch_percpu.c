...
static_assert(RISCV_PERCPU_HARTID == offsetof(loongarch_percpu_t, hartid),
              "RISCV_PERCPU_HARTID offset is wrong");
static_assert(LOONGARCH_PERCPU_IRQSTACK == offsetof(loongarch_percpu_t, irq_stack),
              "LOONGARCH_PERCPU_IRQSTACK offset is wrong");

/****************************************************************************
 * Private Data
 ****************************************************************************/

static loongarch_percpu_t   g_percpu[HART_CNT];
...
static void loongarch_percpu_init(void)
{
  uintptr_t  i;
...
{
  loongarch_percpu_t *percpu;
  irqstate_t      flags;

  /* Make sure we are initialized */

  loongarch_percpu_init();

  /* Get free entry for this hart, this must not fail */

  flags = spin_lock_irqsave(&g_percpu_spin);
  percpu = (loongarch_percpu_t *)sq_remfirst(&g_freelist);
  spin_unlock_irqrestore(&g_percpu_spin, flags);
  DEBUGASSERT(percpu);
  ...
  WRITE_CSR64(LOONGARCH_CSR_KS0, percpu);
...
uintptr_t riscv_percpu_get_hartid(void)
{
  ...
  // return ((loongarch_percpu_t *)scratch)->hartid;
}
...
uintptr_t riscv_percpu_get_irqstack(void)
{
  ...
  // return ((loongarch_percpu_t *)scratch)->irq_stack;
}
